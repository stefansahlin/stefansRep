...
class Validation {
	...
	public function checkPersonNummer($uncheckedPersonNummer) {
		...
		if(preg_match($regEx, $uncheckedPersonNummer)) {
			...
			//TODO returnera personnummret i standardformat.
	...
	public function checkDate($uncheckedDate) {
		...
		if(preg_match($regEx1, $uncheckedDate)||preg_match($regEx2, $uncheckedDate)||preg_match($correctRegEx, $uncheckedDate)) {
			...
			//Kollar om det returnerade formatet stämmer överns med standardformatet.
			if(preg_match($correctRegEx,$correctformat)) {
...
//USERNAME
		...
		if(!$this->checkUsername($username)) {
			...
			echo "USERNAME i rätt format adolf, går inte igenom";
			return false;
		}
		$username = "ado";
		//För kort USERNAME

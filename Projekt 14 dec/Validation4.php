...
class Validation {
	...
	public function FormatTextString($string, $allowed = false){
		$string = preg_replace('@<script[^>]*>.+</script[^>]*>@i', "", $string); 
			 
		if($allowed){
			$string = strip_tags($string, "<p><b><i><u>");
			$string = htmlentities($string);
			$search = array('/&lt;p.*?&gt;/isu', '/&lt;\/p&gt;/isu', '/&lt;b.*?&gt;/isu', '/&lt;\/b&gt;/isu', '/&lt;i.*?&gt;/isu', '/&lt;\/i&gt;/isu', '/&lt;u.*?&gt;/isu',  '/&lt;\/u&gt;/isu'); 
			$replace = array('<p>', '</p>', '<b>', '</b>', '<i>', '</i>', '<u>', '</u>');

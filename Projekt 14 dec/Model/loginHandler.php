...
class LoginHandler{
	...
	public function RemoveUserSession(){
 	...
 	public function Remove($userId) {
        ...
        $stmt = $this->m_mysqli->prepare("DELETE FROM `members` WHERE UserId = ?"); 
	...
	{
		$stmt = $this->m_mysqli->prepare("SELECT * FROM `members` WHERE `Username` = ? AND `Password` = ?");
	...
	public function GenerateCookiePassword($username){
		...
		$stmt = $this->m_mysqli->prepare("UPDATE members SET tempPassword = ? WHERE Username = ?");
	...
	public function DoCookieLogin($username, $password){		
	  	...
	  	$sql = "SELECT * FROM `members` WHERE `Username` = ? AND `tempPassword` = ?"; 

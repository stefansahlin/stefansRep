...
class ChangeAuthorityModel{
	...
	public function GetAllMembers(){
		...
		$membersArray = Array();			
		$query = "SELECT Username, P_Id FROM Members2";	//Plocka här in bindparam.
		$result = $this->m_mysqli->query($query);
		while ($row = $result->fetch_array()){
			$membersArray[] = $row;
			//Eventuellt felmeddelande om det inte finns några medlemmar.
		} 
		return $membersArray;
	}
	...
	public function BlockedEnemies($userId){
		$enemies =  Array();
		$stmt = $this->m_mysqli->prepare("SELECT `BlockedId` FROM access2 WHERE `UserId` = ?");	//BindParam	
